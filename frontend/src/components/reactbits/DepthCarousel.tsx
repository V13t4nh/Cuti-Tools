// Official React Bits: https://reactbits.dev/components/depth-carousel
import {
  useCallback,
  useEffect,
  useMemo,
...
export type DepthCarouselItem = string | { image: string; alt?: string; fallback?: string };
...
const DepthCarousel = ({
  ...
  const handleCardMouseMove = useCallback(
    (e: React.MouseEvent<HTMLDivElement>, index: number) => {
      if (index !== active || dragRef.current?.moved) {
      ...
      const imgSrc = currentImg?.currentSrc || currentImg?.src || data[index]?.image || '';
      loupe.style.backgroundImage = `url("${imgSrc}")`;

      const zoom = 2.6;
      const radius = 125;
      const bgX = -mx * zoom + radius;
      const bgY = -my * zoom + radius;

      loupe.style.opacity = '1';
      loupe.style.transform = `translate(${mx - radius}px, ${my - radius}px)`;
          ...
          >
            <img
              ...
              onError={(e) => {
                ...
                if (item.fallback && !target.dataset.hasFallback) {
            ...
            {active === i && (
              <div
                ref={loupeRef}
                className="depth-carousel__loupe"
              />
            )}
          </div>
        ))}
