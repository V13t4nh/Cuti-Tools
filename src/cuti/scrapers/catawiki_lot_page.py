...
class _PageParser(HTMLParser):
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if self._item_depth:
            self._item_depth -= 1
            if not self._item_depth and (value := _clean(" ".join(self._item))):
...
def _material(value: str | None) -> str | None:
    ...
    if "gold" in text or re.search(r"\b\d{2}\s*k\b", text):
        return "gold"
    return "steel" if any(token in text for token in ("steel", "stainless")) else "other"
...
def parse_lot_page(html: str, *, rules: Rules | None = None) -> LotDetails:
    ...
    parser.feed(html)
    parser.close()
    specs: dict[str, str] = dict(next_specs)
    ...
    for key, value in specs.items():
        ...
        if field is not None and typed[field] is None:
            typed[field] = value
    ref = typed["ref_number"]
    if isinstance(ref, str):
        cleaned_ref = re.sub(r"^(?:ref(?:erence)?\.?\s*[:#-]?\s*)", "", ref, flags=re.I)
        typed["ref_number"] = cleaned_ref or ref
    dom_desc = _clean(" ".join(parser._description))
    description = next_desc or dom_desc
    caliber = typed["caliber"]
    case_code = typed["case_code"]
    if rules is not None:
        from ..normalize_identity import split_identity

        ident = split_identity(str(typed["brand"] or ""), typed["ref_number"],
                               title=str(typed["model"] or ""), description=description or "", rules=rules)
        caliber = caliber or ident.caliber
        case_code = case_code or ident.case_code
    return LotDetails(
